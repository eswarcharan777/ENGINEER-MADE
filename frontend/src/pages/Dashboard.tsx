...
const featureMenu = [
  ['overview','📊','Dashboard'], ['ai-tutor','🤖','AI Engineering Tutor'], ['doubt-upload','📷','Code & Diagram Doubts'], ['recommendation','✨','Recommendations'], ['continue','▶','Continue Learning'], ['quiz','🧠','Module Quizzes'], ['flashcards','🗂️','Flashcards'], ['focus-timer','🍅','Focus Timer'], ['focus-music','🎵','Focus Music'], ['doubt-forum','👥','Doubt Forum'], ['refer-friends','🤝','Refer Friends'], ['themes','🌈','Themes'], ['my-story','🤖','My Story'], ['wallpapers','🖼️','Wallpapers'], ['performance','📈','Performance'], ['badges','🏅','Badges'], ['loot-box','🎁','Study Prompt Box'], ['whiteboard','🎨','Whiteboard'], ['coding','💻','Coding Practice'], ['projects','🚀','Projects'], ['certificates','🏆','Certificates'], ['bookmarks','🔖','Bookmarks'], ['notes','📝','Notes'], ['resume','📄','Resume Builder'], ['reports','🚩','Report Content'], ['goals','🎯','Weekly Goals'], ['schedule','⏰','Schedule Task'], ['languages','🌐','Languages'], ['assessment','🧩','Skill Assessments'], ['groups','👥','Study Groups'], ['simulations','⚙️','Simulations'], ['interviews','🎙️','Mock Interviews'], ['repo-analysis','🔎','Repository Analysis'], ['analytics','📈','Advanced Analytics'], ['skill-tree','🌳','Skill Tree'], ['adaptive-practice','🎯','Adaptive Practice'], ['weakness-heatmap','🗺️','Weakness Heatmap'], ['daily-challenge','⚡','Daily Challenge'], ['ai-summarizer','🧾','AI Summarizer'], ['cheatsheet','🗒️','Cheatsheet Generator'], ['career-explorer','🧭','Career Explorer'], ['job-readiness','📐','Job Readiness Predictor'], ['placement','🔒','Placement Preparation · Coming soon'],
];
...
export default function Dashboard() {
  ...
  const continueTitle = stats.next?.title || `Start your ${selectedRole} roadmap`;
  const continueSubtitle = stats.next ? `${selectedRole} · Next lesson` : `${selectedRole} · Your first lesson awaits`;
  const hasContinueLesson = Boolean(stats.next);
  const assessmentDays = new Set(assessmentAttempts.map(item => new Date(item.completedAt).toDateString()));
  const studyDays = Array.from({ length:84 }, (_, index) => { const day = new Date(); day.setDate(day.getDate() - (83 - index)); return { key:day.toDateString(), active:assessmentDays.has(day.toDateString()) }; });
  ...
  return <div className="dashboard-shell dashboard-v2">
    ...
    <main className="dashboard-workspace"><header className="dashboard-topbar"><button className="sidebar-toggle" onClick={() => setSidebarOpen(value => !value)}>☰</button><h2>{activeLabel}</h2><div className="topbar-level" aria-label={`Level ${level}, ${levelXp} percent toward the next level`}><b>LVL {level}</b><span><i style={{ width: `${levelXp}%` }} /></span></div><div className="topbar-profile"><div><b>{profile.name || user.displayName || 'Engineer'}</b><span>Level {level} · {selectedRole}</span></div><i>{initials}</i></div></header>
      <div className="dashboard-page" id="overview">{activeFeature === 'overview' ? <>
        ...
        <section className="study-activity"><div className="panel-heading"><h2>🗓️ Study Activity</h2><span>{assessmentDays.size} active assessment day{assessmentDays.size === 1 ? '' : 's'} · last 12 weeks</span></div><div className="study-activity-grid" aria-label="Assessment activity over the last 12 weeks">{studyDays.map(day=><i key={day.key} className={day.active ? 'active' : ''} title={day.key} />)}</div><small>Activity is recorded when you complete a skill assessment.</small></section>
        <section className="continue-card"><div><span className="dashboard-kicker">CONTINUE YOUR ROADMAP</span><h2>{continueTitle}</h2><p>{continueSubtitle}</p></div>{hasContinueLesson ? <Link className="btn btn-secondary" to={`/learn/${selectedPath?.id}/${stats.next?.moduleId}/${stats.next?.id}`}>Continue lesson →</Link> : <button className="btn btn-secondary" onClick={() => window.location.reload()}>Load roadmap →</button>}</section>
